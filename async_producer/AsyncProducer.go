/**
 This is not production quality code - it's only an example.
 All ASYNC events would need to be handled for it to be ready for prime time
 */
package main
...
func main() {
	...
	config.Consumer.Offsets.Retention = time.Hour * 12
...
*/
	...
	defer func() {
        ...
        start := time.Now();
	message.Value = sarama.StringEncoder(*value + strconv.Itoa(90))
	for i:=0; i< *count; i++ {
        ...
        fmt.Printf("time taken to publish=%v\n", finish)
        fmt.Printf("thruput=%f msgs/sec\n", (float64)(*count)/(finish.Seconds()))
}
